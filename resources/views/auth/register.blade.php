...
@section('content')
<div class="bg-primary/10 flex h-auto min-h-screen items-center justify-center overflow-x-hidden py-10">
    <div class="relative flex items-center justify-center px-4 sm:px-6 lg:px-8">
        <div class="absolute">
            <svg width="612" height="697" viewBox="0 0 612 697" fill="none" style="scale: 1.3 -1.2;" xmlns="http://www.w3.org/2000/svg">
                <path
                ...
                <defs>
                    <linearGradient
                        ...
                        <stop offset="1" stop-color="var(--color-primary)" stop-opacity="0.4" />
                    </linearGradient>
                </defs>
            </svg>
        </div>
        <div class="bg-base-100 shadow-base-300/20 z-1 w-full space-y-6 rounded-xl p-6 shadow-md sm:min-w-md max-w-lg lg:p-8">
            ...
            <div class="space-y-4">
                <form class="mb-4 space-y-4" method="post" action="/users">
                    @csrf
                    <div>
                        <label class="label-text after:content-['*'] after:text-red-500 after:ml-1" for="userLastname">
                            Apellidos:
                        </label>
                        <input type="text" placeholder="Ingresa tus apellidos" class="input" id="userLastname" name="user_lastname" value="{{ old('user_lastname')}}" />
                        @if ($errors->has('user_lastname'))
                        <p class="text-red-500 text-sm">{{ $errors->first('user_lastname') }}</p>
                        @endif
                    </div>
                    <div>
                        <label class="label-text after:content-['*'] after:text-red-500 after:ml-1" for="userName">
                            Nombre:
                        </label>
                        <input type="text" placeholder="Ingresa tu nombre" class="input" id="userName" name="user_name" value="{{ old('user_name') }}" />
                        @if ($errors->has('user_name'))
                        <p class="text-red-500 text-sm">{{ $errors->first('user_name') }}</p>
                        @endif
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="label-text after:content-['*'] after:text-red-500 after:ml-1" for="userPhone">
                            Teléfono:
                        </label>
                        <input placeholder="Ingresa tu teléfono" name="user_phone" class="input" id="userPhone" value="{{ old('user_phone') }}" />
                        @if ($errors->has('user_phone'))
                        <p class="text-red-500 text-sm">{{ $errors->first('user_phone') }}</p>
                        @endif
                    </div>
                    <div>
                        <label class="label-text after:content-['*'] after:text-red-500 after:ml-1" for="userAddress">
                            Dirección:
                        </label>
                        <input placeholder="Ingresa tu dirección" name="user_address" class="input" id="userAddress" value="{{ old('user_address') }}" />
                        @if ($errors->has('user_address'))
                        <p class="text-red-500 text-sm">{{ $errors->first('user_address') }}</p>
                        @endif
                    </div>
                    <div>
                        ...
                        <div class="input">
                            <input id="userPassword" type="password" placeholder="***********" name="user_password" />
                            <button
                                type="button"
                    ...
                    <div>
                        <label class="label-text after:content-['*'] after:text-red-500 after:ml-1" for="passwordConfirm">
                            Confirmar contraseña:
                        </label>
                        <div class="input">
                            <input id="passwordConfirm" type="password" placeholder="***********" name="password_confirm" />
                            <button
                                type="button"
                                data-toggle-password='{ "target": "#passwordConfirm" }'
                                class="block cursor-pointer"
                        ...
                        <p class="text-red-500">{{ $errors->first('password_confirm') }}</p>
                        @endif
                    </div>

                    <div>
                        <div class="flex items-center gap-2">
                            <input type="checkbox" class="checkbox checkbox-primary rounded size-4" id="policyagreement" value="1" name="policy_agreement" />
                            <label class="label-text text-base-content/80 p-0 text-sm" for="policyagreement">
                                Acepto la
                                <a href="#" class="link link-animated link-primary font-normal">política de privacidad y los términos</a>
                            </label>
                        </div>
                        @if ($errors->has('policy_agreement'))
                        <p class="text-red-500">{{ $errors->first('policy_agreement') }}</p>
                        @endif
                    </div>
                    @if (session('error'))
                    <p class="text-red-500 text-sm">{{ session('error') }}</p>
                    @endif
                    <button class="btn btn-lg btn-primary btn-block rounded-lg">Registrarse</button>
                </form>
                <p class="text-base-content/80 mb-4 text-center">
                    ¿Ya tienes una cuenta?
                    <a href="/login" class="link link-animated link-primary font-normal">Inicia sesión</a>
                </p>
                <div class="divider">o</div>
                <button class="btn btn-text btn-block rounded-lg bg-base-300/5 hover:bg-base-300/10">
